...
| POC-010 | Infrastructure | Deploy Node.js backend (CORS proxy + Graph API) to App Service free tier OR use Static Web Apps backend API | P1 | Blocked |  |  | POC-006 | Blocked: Subscription quota limit (0 VMs) prevents App Service creation. Alternative: App Service free tier unavailable in subscription. Workaround: Skip backend for PoC; test core passkey flow without Graph API auto-enroll/passkey-count features. Backend deferred to PROD phase. |
...
| POC-012 | Secrets | Store client secret in Static Web Apps configuration (or App Service .env) | P1 | Blocked |  |  | POC-010, POC-011 | Blocked: Static Web Apps does not require runtime secret config for React SPA (build-time env vars only); app currently references VITE_APP_SECRET but SPA is static-hosted (no backend). Mitigation: Use CORS proxy backend app (deferred) or remove Graph API calls for PoC. |
...
| POC-015 | Validation | Smoke test: Sign in with passkey (Windows Hello or FIDO2) | P0 | In Progress | 2026-08-12 |  | POC-014 | 🔄 IN PROGRESS: Tested g.petch@gmail.com; CIAM progressed to password entry. No direct passkey option at this step (per known platform limitation). Continuing via password path to reach MFA/verify step. Workaround: App orange banner provides "Switch to passkey" redirect. Full flow test pending. |
| POC-016 | Validation | Verify phishing-resistant banner state (GREEN) for passkey sign-in | P0 | Not Started |  |  | POC-015 | Banner displays GREEN with text "Phishing-resistant MFA active"; AMR claim contains `hwk`, `fido`, or `ngcmfa` |
...
| POC-019 | Graph API | Verify passkey count fetched from Graph (GET /users/{id}/authentication/fido2Methods) | P1 | Not Started |  |  | POC-016 | Passkey count displayed on SecurityPage; matches actual user registration count |
...
| POC-021 | Validation | Test bootstrap flow: first-time user registers passkey with OTP (passkeyCount === 0 → allow OTP) | P1 | Not Started |  |  | POC-015 | New user with no passkeys can use OTP to unlock passkey registration; blue info banner displayed; registration succeeds |
| POC-022 | Validation | Confirm passkey works across three methods: Windows Hello, hardware FIDO2, phone synced passkeys | P1 | Not Started |  |  | POC-016 | Test each of three passkey types (hwk, fido, pop/swk); all three produce correct AMR claims; all three reach green banner |
...
| POC-027 | Known Issues | Document platform limitations encountered during PoC | P1 | Done | 2026-08-12 |  | POC-025 | Issues documented in POC-TEST-RUNBOOK.md Appendix (4 known limitations from earlier sessions); confirmation needed during manual testing that all 4 limitations still present in current deployment. |
...
| POC-030 | Decision | Approve PoC completion and graduation to PROD deployment | P0 | Blocked / In Progress |  |  | POC-025, POC-026 | [DECISION GATE] Pending manual test completion (POC-025, POC-026). Exit criteria: 7/8 pass (passkey enrollment ≥95%, green banner ≥95%, Graph API 100%, bootstrap 100%, 3 passkey types working, HTTPS valid, ≥4/5 feedback, 0 critical security issues). If pass, go to PROD-DEPLOYMENT-BACKLOG.md. If fail, fix blockers and re-test. |
