...
| POC-015 | Validation | Smoke test: Sign in with passkey (Windows Hello or FIDO2) | P0 | Blocked | 2026-08-12 |  | POC-014 | 🔴 BLOCKER: 400 Bad Request on CIAM token endpoint POST /oauth2/v2.0/token after password entry. Request ID: 019ff31c-10c8-7211-98d0-1d5455b28d1e. Likely causes: PKCE code_verifier mismatch, redirect URI mismatch, state validation failure, or missing grant_type parameter. Next: Inspect token request payload (Payload tab in DevTools) to diagnose. If unresolvable, escalate to CIAM platform team. |
