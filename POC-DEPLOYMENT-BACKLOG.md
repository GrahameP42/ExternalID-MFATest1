...
| POC-021 | Validation | Test bootstrap flow: first-time user registers passkey with OTP (passkeyCount === 0 → allow OTP) | P1 | Blocked | 2026-08-12 |  | POC-015 | ⚠️ KNOWN PLATFORM LIMITATION: Passkey registration fails on Static Web Apps domain due to WebAuthn RP ID domain mismatch. App runs on azure static web apps domain; CIAM expects passkeys to be registered against ciamlogin domain. Browser rejects cross-domain registration (security constraint). Workaround for PoC: Use existing registered passkeys only (fidotest1 has Hyper FIDO Pro). For production: deploy passkey ceremony on custom domain via Front Door. Skip this test for PoC; focus on sign-in validation. |
