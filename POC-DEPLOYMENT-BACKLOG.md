...
| POC-015 | Validation | Smoke test: Sign in with passkey (Windows Hello or FIDO2) | P0 | In Progress | 2026-08-12 |  | POC-014 | 🔄 IN PROGRESS: Tested g.petch@gmail.com via Playwright browser (Chrome-based); progressed to password entry successfully (no direct passkey option per platform limitation). ⚠️ ISSUE: Edge InPrivate mode fails to progress past email entry (likely third-party cookie/storage restriction in InPrivate). Workaround: Use regular (non-InPrivate) browser session or Chrome/Chromium for testing. Full password→MFA→banner flow pending completion in regular browser mode. |
